...
namespace Finance\Transfers\Code\Models;

defined('KAZIST') or exit('Not Kazist Framework');

use Kazist\Model\BaseModel;
use Kazist\KazistFactory;
use Finance\Payments\Code\Models\PaymentModel;
...
class TransfersModel extends BaseModel {
    ...
    public function getQueryBuilder() {
        ...
        $query = parent::getQueryBuilder('#__finance_transfers', 'ft');
    ...
    public function save($form) {
        ...
        if (!WEB_IS_ADMIN) {
            ...
            $url = $this->generateUrl('finance.transfers');
        ...
        if ($total_param->amount > $total_amount) {
            ...
            $factory->redirect('finance.transfer.transfer');
        }
    ...
    public function getGiveCommission($origin_user_id, $target_user_id, $amount, $transfer_id, $params) {
        ...
        if (!empty($params)) {
            foreach ($params as $key => $param) {
                ...
                $id = $factory->saveRecord('#__finance_transactions', $data_obj);
    ...
    public function sendEmail($origin_user_id, $target_user_id, $transfer_id) {
        ...
        $tmp_array['transfer'] = $factory->getRecord('#__finance_transfers', 'ft', array('id=:id'), array('id' => $transfer_id));

        $email->sendDefinedLayoutEmail('finance.transfers.fundtranfered', $tmp_array['user']->email, $tmp_array);
        $email->sendDefinedLayoutEmail('finance.transfers.fundreceived', $tmp_array['target_user']->email, $tmp_array);
    }
    ...
    public function getAllowedIn() {
        ...
        $query = new Query();
        $query->select('fgn.*');
        $query->from('#__finance_transfers_allowedin', 'fgn');
    ...
    public function reverseTransfer() {
        ...
        $factory->saveRecord('#__finance_transfers', $data_obj);
    }
    ...
    public function saveSettingTransfer() {
        ...
        if (!empty($country_ids)) {
            foreach ($country_ids as $country_id) {
                ...
                $ids_arr[] = $factory->saveRecord('#__finance_transfers_allowedin', $data_obj, $where_arr, $data_obj);
            }
        }

        if (!empty($ids_arr)) {
            $query = new Query();
            $query->delete('#__finance_transfers_allowedin');
